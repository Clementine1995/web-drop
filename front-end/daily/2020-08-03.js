...
// 添加Promise函数对象上的方法 allSettle
// 只有等到参数数组的所有 Promise 对象都发生状态变更（不管是fulfilled还是rejected），返回的 Promise 对象才会发生状态变更,一旦发生状态变更，状态总是fulfilled，不会变成rejected

custPromise.allSettled = function (promises) {
  return new custPromise((resolve, reject) => {
