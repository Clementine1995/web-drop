...
有一个点需要注意的是，全局作用域与块级作用域中的let const 在声明前访问有不同，全局作用域下访问 `VM92:1 Uncaught ReferenceError: xxx is not defined`，而在局部作用域中则会提示 `VM104:2 Uncaught ReferenceError: Cannot access 'xxx' before initialization` 。
