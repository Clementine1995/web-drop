...
1. 工厂模式，通过new Object出的实例来扩展属性，调用时不使用new,创建的对象无法识别，都是Object的实例。
...
2. 经典继承（借用构造函数），在子级中通过apply调用父级构造函数，Parent.call(this)，因为是传递的子级实例this,来解决原型链继承存在共享等问题，缺点就是方法都定义在构造函数中，每创建一次实例都会创建一次方法，而且无法访问到父级原型上的属性
