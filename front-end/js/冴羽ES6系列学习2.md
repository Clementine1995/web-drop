...
CommonJS 能够比 ES modules 的不同是，通过文件系统去加载文件，这比从网上下载文件要花的时间少得多。这就意味着，Node 将会在加载文件的时候阻塞主线程。只要文件加载完成，它就会去实例化并且去做求值操作（这也就是 CommonJS 不会在各个独立阶段去做的原因）。这同样说明了，当在返回模块实例之前，就会遍历整个依赖关系树然后去完成加载、实例化以及对各个依赖进行求值的操作。

注意，在 Node 中使用 CommonJS 模块时，在模块声明中是可以使用变量的，因为在查找下一个模块之前，将执行完这个模块所有的代码（直到通过require去返回这个声明）。这就意味着这些变量将会在处理模块解析时被赋值。

但是在 ES modules 中，将在执行模块解析和进行求值操作前就建立好整个模块依赖关系图表。这也就是说在模块声明时，不能去使用这些变量，因为这些变量那时还并没有被赋值。
...
为了实例化模块图，引擎将进行深度优先遍历。这意味着它将下降到图表的底部-直到不依赖其他项的依赖项-并设置其导出。
...
相反，ES模块使用一种称为实时绑定的东西。两个模块都指向内存中的相同位置。这意味着，当导出模块更改值时，该更改将显示在导入模块中。导出值的模块可以随时修改导出的值，而导入模块的不能改变它们导入的值，但是如果导入的是一个对象，那对象属性是可以更改的。之所以拥有这样的实时绑定，是因为可以在不运行任何代码的情况下连接所有模块。当具有循环依赖性时，这将有助于求值。因此，在此步骤结束时，已连接了所有实例以及导出/导入变量的存储位置。

##### 求值操作

最后一步是对内存中的相关区域进行填充。JS 引擎是通过执行顶层代码去完成这件事的——在函数外的代码。

除了对内存中相关进行填充外，对代码进行求值也会造成副作用。比如说，模块可能会去调用一个服务。因为潜在的副作用，所以想要只对模块进行一次求值。通过使用**模块映射集合**缓存每个URL，因此每个模块只有一个模块记录，这就保证了每一个模块只被执行一次。和实例化的过程一样，它同样采用的是深度优先后序遍历的方法。
